...
const getFeaturedEvents = async () => {
    try {
        ...
        let response = await fetch(`http://localhost:3000/api/events/featured`, {
            cache: "no-store"
        })
  ...
  return (
    ...
    <Container w_class={"max-w-[60rem]"}>
                    ...
                    <div className='flex overflow-x-hidden' ref={featuredEventsEl}>
                        {
                            ...
                            <div key={event._id} style={{flex:"0 0 100%"}}>
                                ...
                                <div className="flex max-w-[65rem] border-2 border-blue-900 rounded">
                                    
                                    <div className='w-[100%] md:w-[65%]'>
                                        <a href={`http://localhost:3000/event/${event.slug}/${event._id}`} target='_blank'>
                                            <img className='rounded w-[250px] h-auto' src={event.image} />
                                        </a>
                                    </div>
                                    ...
                                    <div className='p-6 py-5 hidden md:block md:w-[38%]'>
                                        ...
                                        <div className='flex justify-center'>
                                            <a className='w-[100%]' href={`http://localhost:3000/event/${event.slug}/${event._id}`} target='_blank'>
                                                <button className='rounded hover:text-white hover:border-blue-600 hover:bg-blue-500 duration-200 transition ease-in-out py-2 px-3 w-[100%] uppercase bg-white text-gray-700 border border-blue-900'>More details</button>
                                            </a>
                                        </div>
                                    </div>
...

        {
            ...
            <div className='mt-20 border border-red-500 '>
                Loading events...
            </div>
        }
    </Container>
