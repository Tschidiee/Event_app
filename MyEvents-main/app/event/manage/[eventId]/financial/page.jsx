...
const getFinancialInfo = async (eventId) => {
    ...
    let financialInfo = await fetch(`http://localhost:3000/api/events/${eventId}/financial`, {
        cache: 'no-store',
        headers:{
                                ...
                                <div className="flex items-center justify-between gap-2">
                                    <div>
                                        ...
                                        <p className={`${ticket.numberOfSales ? 'text-green-500' : 'text-gray-500'} font-bold text-2xl`}>{
                                            (ticket.price * ticket.numberOfSales).toLocaleString('en-NG', {style:'currency', currency: 'NGN'})
                                        }</p>
                                    </div>
                                    <div>
